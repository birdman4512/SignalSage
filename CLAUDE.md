...
- `digest.seed_vote_reactions` — `true` (default) makes the bot pre-add 👍/👎 to each digest story so rating is one click; `false` leaves the reactions to users (their 👍/👎 still count as feedback). Applies to stories queued after the change
